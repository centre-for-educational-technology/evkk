...
def arvuta(inputText):
    ...
    singularAdjectives = feat_ratio(adjectiveFeats, "Number=Sing", words) # ainsuses omadussõnade osakaal (koondmudelis)
    ...
    lemmasNoProperNouns = text[text.Upos != "PROPN"].groupby("Lemma").Lemma.count().to_frame() # pärisnimede kõrvalejätmine
    ...
    sumFeatValues = np.array([lemmas, TTR, MaasIndex, avgWord, person3Verbs, elativeNouns, nounAbstractness,
        person1Verbs, pluralPronouns, rarerThan5000MostFreq, rarerThan2000MostFreq, adjectiveCases,
