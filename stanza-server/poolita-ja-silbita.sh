...
#
# Prerequisites:
# - vmeta and et.dct for splitting compound words (precompiled; run scripts/compile-vabamorf.sh to recompile)
# - silbitaja.hfst_olw for syllabification (precompiled; run scripts/compile-silbitaja.sh to recompile)
#
#
# Standard input:
...
hfst-lookup -q silbitaja.hfst_olw |                                   # Run silbitaja.hfst_olw to mark syllable boundaries (see HFST's documentation for flag's explanation)
cut -f 2 |                                                            # Only preserve the content in the second column
