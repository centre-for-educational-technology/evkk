...
public class WordContextService {
  ...
  public List<WordContextResponseDto> getWordContextResponse(WordContextRequestDto dto) {
    ...
    List<WordAndPosInfoDto> wordlist = dto.getType().equals(WORDS)
      ...
      : removeCapitalization(sanitizeLemmaList(asList(stanzaServerClient.getLemmadJaPosInfo(sanitizedTextContent))), dto.isKeepCapitalization());
    return generateContextList(wordlist, dto, sanitizedTextContent);
  }

  private List<WordContextResponseDto> generateContextList(List<WordAndPosInfoDto> wordlist, WordContextRequestDto dto, String sanitizedTextContent) {
    List<WordContextResponseDto> result = new ArrayList<>();
    for (int i = 0; i < wordlist.size(); i++) {
      ...
      if (wordlist.get(i).getWord().equals(keyword)) {
        ...
        result.add(new WordContextResponseDto(
